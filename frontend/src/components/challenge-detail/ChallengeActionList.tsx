...
const initializeActionsWithStatus = (actions: Omit<Action, 'challenges'>[]): ActionWithStatus[] => {
...
export const 	ChallengeActionList = ({ challengeId, activeTab }: ActionList2Props) => {
  const { data, loading, error } = useGetActionsByChallengeIdQuery({
    variables: { challengeId },
  });

  const [actionsWithStatus, setActionsWithStatus] = useState<ActionWithStatus[]>([]);

  useEffect(() => {
    const actions = data?.getActionsByChallengeId ?? [];
    setActionsWithStatus(initializeActionsWithStatus(actions));
  }, [data]);
    ...
    <ul className="flex flex-col gap-2 sm:grid sm:grid-cols-3 md:gap-3 xl:grid-cols-4 xl:gap-4">
      {filteredActions.length > 0 ? (
        filteredActions.map((action) => (
          <li key={action.id}>
            <ChallengeActionCard action={action} onToggleStatus={handleToggleStatus} />
          </li>
        ))
      ) : (
